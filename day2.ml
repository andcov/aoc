...
let line_to_game l = 
  ...
  let rounds = List.map rounds ~f:(fun rnds -> 
    String.split ~on:',' rnds |>
    List.fold ~init:{red = 0; green = 0; blue = 0} ~f:(fun acc rnd ->
      let _ = Str.search_forward (Str.regexp {|\([0-9]+\) \([a-z]+\)|}) rnd 0 in
...
let round_is_possible round = round.red <= 12 && round.green <= 13 && round.blue <= 14
let game_is_possible game = List.for_all game.rounds ~f:round_is_possible

let min_cube_set game = List.fold game.rounds ~init:{red = 0; green = 0; blue = 0} 
  ~f:(fun acc round -> {red = Int.max acc.red round.red; green = Int.max acc.green round.green; blue = Int.max acc.blue round.blue})
...
let () = 
    ...
    let sum1 = List.filter games ~f:game_is_possible |> List.fold ~init:0 ~f:(fun acc game -> acc + game.id) in
